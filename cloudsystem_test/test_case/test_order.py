...
from cloudsystem_test.config.deskfile import clear_yaml, read_yaml
...
def test_case4():
        ...
        test_CP().test_AForwardordering3(renewOrderNo, channel)
        test_CP().test_AForwardordering4()
...
def test_case5():
        ...
        test_CP().test_Thirddirectordering(account, productItemCode, thirdOrderNo, renewOrderNo, orderType,
                                           channelCode)
        test_CP().test_Thirddirectordering2()
        test_CP().test_Thirddirectordering3()

@allure.title("用例编号：CJHF2 用例名称：营销订购")#描述：针对订购云机模块-营销订购功能测试
def test_case6():
        account = 15013957569
        productItemCode = 100023635876
        thirdOrderNo = randint(9, 99999999999999999999999999)
        renewOrderNo = ''
        orderType = "ORDER_BUY"  # 订购：ORDER_BUY，续订：ORDER_RENEW
        channelCode = "782976529302"
...
def test_case7():
        phone = 15013957569
        goodsId = "100058896259"
        oprType = "01"
        if oprType == "01":
            clear_yaml()
            outOrder = randint(9, 99999999999999999999999999)
            # print(outOrder)
        elif oprType == "03":
            ...
            print("格式有误！")
        outOrderId = int(outOrder)
        notifyType = 1
        notifyiResult = 0
        expireTime = "20241129145900"
        effectTime = "20241120185900"


        test_CP().test_Reverse_order(phone)
        test_CP().test_Reverse_order2(goodsId, outOrderId, oprType, notifyType, notifyiResult, expireTime,
                                      effectTime)
